...
describe("generateExamUseCase", () => {
  ...
  it("ranks each intent pool before applying distribution when coverage context is absent", () => {
    const availableQuestions: Question[] = [
      {
        ...buildQuestion("t-low", "theoretical"),
        ceIds: ["ce-1"],
        points: 1,
      },
      {
        ...buildQuestion("t-high", "theoretical"),
        ceIds: ["ce-1", "ce-2"],
        points: 5,
      },
      {
        ...buildQuestion("p-low", "practical"),
        ceIds: ["ce-1"],
        points: 1,
      },
      {
        ...buildQuestion("p-high", "practical"),
        ceIds: ["ce-1", "ce-2"],
        points: 5,
      },
    ];
  ...
  it("ranks plain count-only generation by CE richness, then points, then id", () => {
    const availableQuestions: Question[] = [
      { ...buildQuestion("q-c", "theoretical"), ceIds: ["ce-1"], points: 3 },
      { ...buildQuestion("q-a", "theoretical"), ceIds: ["ce-1", "ce-2"], points: 1 },
      { ...buildQuestion("q-b", "theoretical"), ceIds: ["ce-1", "ce-2"], points: 2 },
    ];
  ...
  it("treats CE ids without explicit coverage as a lower-strength eligible match", () => {
    ...
    const availableQuestions: Question[] = [
      {
        ...buildQuestion("q-basic-match", "theoretical"),
        ceIds: ["ce-1"],
      },
      {
        ...buildQuestion("q-strong-match", "theoretical"),
        ceIds: ["ce-1"],
        ceCoverage: [{ ceId: "ce-1", score: 0.9 }],
      },
    ];
    ...
    expect(output.errors).toEqual([]);
    expect(output.exam).toBeDefined();
    expect(output.exam?.questions[0].id).toBe("q-strong-match");
  });
  ...
  it("uses CE ids as the eligibility gate when explicit coverage references an unrelated CE", () => {
    ...
    const availableQuestions: Question[] = [
      {
        ...buildQuestion("q-ineligible-coverage", "theoretical"),
        ceIds: [],
        ceCoverage: [{ ceId: "ce-1", score: 0.95 }],
      },
      {
        ...buildQuestion("q-eligible-basic", "theoretical"),
        ceIds: ["ce-1"],
      },
    ];
    ...
    expect(output.errors).toEqual([]);
    expect(output.exam).toBeDefined();
    expect(output.exam?.questions[0].id).toBe("q-eligible-basic");
  });
});
